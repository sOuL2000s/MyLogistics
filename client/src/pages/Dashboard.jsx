  ...
  return (
    <div className="container mx-auto animate-slide-up">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mb-10">
        <Card className="!bg-primary text-white border-none flex items-center gap-4 md:gap-5 p-6 md:p-8">
           <div className="bg-white/20 p-3 md:p-4 rounded-2xl flex-shrink-0"><FaBoxOpen className="text-2xl md:text-3xl" /></div>
           <div className="min-w-0">
             <p className="text-primary-light font-bold text-xs md:text-sm uppercase tracking-wider">Total Shipments</p>
             <h3 className="text-2xl md:text-3xl font-black truncate">{shipments.length}</h3>
           </div>
        </Card>
        <Card className="flex items-center gap-4 md:gap-5 p-6 md:p-8">
           <div className="bg-amber-50 p-3 md:p-4 rounded-2xl flex-shrink-0"><FaClipboardList className="text-2xl md:text-3xl text-amber-500" /></div>
           <div className="min-w-0">
             <p className="text-slate-400 font-bold text-xs md:text-sm uppercase tracking-wider">Pending</p>
             <h3 className="text-2xl md:text-3xl font-black text-slate-800 truncate">{totalPending}</h3>
           </div>
        </Card>
        <Card className="flex items-center gap-4 md:gap-5 p-6 md:p-8">
           <div className="bg-emerald-50 p-3 md:p-4 rounded-2xl flex-shrink-0"><FaWallet className="text-2xl md:text-3xl text-emerald-500" /></div>
           <div className="min-w-0">
             <p className="text-slate-400 font-bold text-xs md:text-sm uppercase tracking-wider">Total Spent</p>
             <h3 className="text-2xl md:text-3xl font-black text-slate-800 truncate">${totalSpent.toFixed(2)}</h3>
           </div>
        </Card>
      </div>
